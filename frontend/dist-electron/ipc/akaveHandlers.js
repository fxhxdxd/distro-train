...
export function registerAkaveHandlers() {
    ipcMain.handle('akave:configure', (_event, { awsAccessKeyId, awsSecretAccessKey, }) => {
        return akaveService.configureAws(awsAccessKeyId, awsSecretAccessKey);
    });
    ipcMain.handle('akave:uploadFile', (_event, filePath) => {
        return akaveService.uploadFile(filePath);
    });
    ipcMain.handle('akave:uploadDataset', (event, filePath) => {
    ...
    ipcMain.handle('akave:fetchFile', (_event, objectKey) => {
